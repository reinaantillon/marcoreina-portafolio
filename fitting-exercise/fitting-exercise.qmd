...
calc_rmse <- function(model, data) {
  ...
  actual <- data$Y
  sqrt(mean((actual - predictions$.pred)^2))
}

# Apply to each model
rmse_null <- calc_rmse(new_model_null, data_clean2)
rmse_model1 <- calc_rmse(new_model1, data_clean2)
rmse_model2 <- calc_rmse(new_model2, data_clean2)
